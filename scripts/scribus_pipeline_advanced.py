...
def apply_meta_to_frames(layout_map, raw_meta, log_file=None):
    """
    Uses sop_to_layout_map/pages/roles in layout_map to:
    ...
    for page_cfg in pages:
        ...
        if page_number is not None:
            try:
                # Handle both ints and strings like "1"
                scribus.gotoPage(int(page_number))
            except Exception as e:
                # If page doesn't exist yet, or conversion fails, skip
                log(f"Could not goto page {page_number}: {e}", log_file)
                continue
